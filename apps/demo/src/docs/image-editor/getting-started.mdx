# Getting Started

Install and set up the Editx Image Editor in your React project.

## Installation

```bash
pnpm add @editx/image-editor
```

Peer dependencies: `react >= 19`, `react-dom >= 19`.

## Basic Usage

```tsx
import { ImageEditor } from "@editx/image-editor";

function App() {
  return (
    <ImageEditor
      src="/photo.jpg"
      onSave={(blob) => {
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "edited.png";
        a.click();
      }}
    />
  );
}
```

## Modal Usage

Wrap the editor in a modal for overlay-style editing:

```tsx
import { useState } from "react";
import { ImageEditorModal } from "@editx/image-editor";

function App() {
  const [open, setOpen] = useState(false);

  return (
    <>
      <button onClick={() => setOpen(true)}>Edit Image</button>
      <ImageEditorModal
        open={open}
        onOpenChange={setOpen}
        src="/photo.jpg"
        onSave={(blob) => console.log("Saved:", blob)}
        onClose={() => setOpen(false)}
      />
    </>
  );
}
```

## Next Steps

- [Configuration](/docs/image-editor/configuration) â€” customize tools, export settings, and more
- [Theming](/docs/image-editor/theming) â€” built-in presets and custom color palettes
- [API Reference](/docs/image-editor/api) â€” full props reference
- [Playground](/playground) â€” try it live with interactive config
