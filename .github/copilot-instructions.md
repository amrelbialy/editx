...
- **Tailwind-first responsive**: Use CSS Container Queries (`@container/editor` + `@3xl/editor:`) â€” never viewport breakpoints or JS detection.
- **Command system**: All document mutations go through the engine command system (undoable). Never mutate engine state directly.
- **Hook ordering**: refs â†’ custom hooks â†’ useState â†’ useMemo/useCallback â†’ useEffect (blank line between groups).
- **No `console.log`** in production code â€” gate behind `__EX_PERF` flag.
