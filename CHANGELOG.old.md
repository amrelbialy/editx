...
## EventAPI: Block Lifecycle Block Lifecycle Events
...
block-based editor has an `EventAPI` ([docs](https://Editx/docs/Editx/js/api/engine/classes/eventapi/)) where you subscribe to specific block IDs and receive typed, bundled events at the end of each engine update cycle.

### Solution

Added an `EventAPI` class to the engine that follows the event-driven pattern. Events are now:
