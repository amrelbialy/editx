...
export function Gamma(this: any, imageData: ImageData): void {
  ...
  for (let i = 0; i < len; i += 4) {
    pixels[i] = Math.min(255, Math.max(0, 255 * Math.pow(pixels[i] / 255, correction)));
    pixels[i + 1] = Math.min(255, Math.max(0, 255 * Math.pow(pixels[i + 1] / 255, correction)));
    pixels[i + 2] = Math.min(255, Math.max(0, 255 * Math.pow(pixels[i + 2] / 255, correction)));
  }
}
