...
import { editorStateToRuns, runsToEditorState, getSelectionOffsets } from './lexical-bridge';
...
describe('lexical-bridge', () => {
  ...
  describe('mixed styles', () => {
    it('preserves adjacent runs with different styles', async () => {
      const runs = [
        run('Hello ', { fontWeight: 'bold' }),
        run('World', { fontStyle: 'italic' }),
      ];
      await applyRuns(runs);
