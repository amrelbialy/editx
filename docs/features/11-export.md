...
| Layer         | What                                                                  | Status             |
| ------------- | --------------------------------------------------------------------- | ------------------ |
| Engine        | `exportScene(options)` method on `RendererAdapter` + `CreativeEngine` | not started        |
...
Zero-flicker approach — Konva's `toDataURL` renders to an internal offscreen canvas, so no screen paint occurs:
...
7. Convert data URL → `Blob`.
8. Return the Blob.

### CreativeEngine facade
...
3. If `onBeforeSave` defined, call it with the blob — use returned blob if not `undefined`.
...
- Pass `isExporting` to `<Topbar>` — button shows spinner + disables during export.
...
1. Load image → apply crop + filter + adjustment + text annotation → Export → `onSave` receives valid Blob.
2. Test PNG, JPEG, WEBP formats — correct MIME types.
...
## Phase C — Export Dialog (deferred)
...
- Quality slider (0.1–1.0, only for jpeg/webp)
- "Export" button
